...
bool SimpleHOTP::validate (uint32_t hotp) {
...
bool SimpleHOTP::validate (uint8_t *hotp) {
	uint32_t toPass = 0;
...
uint32_t SimpleHOTP::generateHOTP (){
  resized_counter = generateHOTP(counter++);
	return resized_counter;
}

void SimpleHOTP::generateStrHOTP (char* hotp) {
	uint32_t int_hotp = generateHOTP();
	char dig_str[6] = "%06lu";
	dig_str[2]=digits+'0'; //1 digit lenght
	sprintf(hotp, dig_str, int_hotp);
}


void SimpleHOTP::generateStrHOTP (uint64_t counter, char* hotp) {
	uint32_t int_hotp = generateHOTP(counter);
	char dig_str[5] = "%06d";
	dig_str[2]=digits+'0'; //1 digit lenght
	sprintf(hotp, dig_str, int_hotp);
}

void SimpleHOTP::saveCounter(){
  EEPROM.put(0,this->counter);
  EEPROM.commit();
  Serial.print("Counter saved: "); Serial.println(uint32_t(this->counter));
}
void SimpleHOTP::getCounter(){
  EEPROM.get(0,this->counter);
  Serial.print("Counter geted: "); Serial.println(uint32_t(this->counter));
}
