...
class SimpleHOTP {
	...
	public:
		...
		uint32_t generateHOTP (uint64_t counter);
    
		uint32_t generateHOTP ();
		void generateStrHOTP (char* hotp);
		void generateStrHOTP (uint64_t counter, char* hotp);
		// generate HOTP as string of fix lenght
		 ...
		 */
		bool validate (uint32_t hotp);
		bool validate (uint8_t *hotp);
